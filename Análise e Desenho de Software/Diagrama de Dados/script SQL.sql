...
CREATE TABLE IF NOT EXISTS `mydb`.`Cliente` (
  ...
  INDEX `fk_Cliente_Endereco1_idx` (`FK_END_IDEND` ASC, `FK_CID_IDCID` ASC),
...
CREATE TABLE IF NOT EXISTS `mydb`.`Profissional` (
  `PK_PRO_CPF` INT NOT NULL AUTO_INCREMENT,
  `PRO_NOME` VARCHAR(100) NOT NULL,
  `PRO_SEXO` CHAR NOT NULL,
  `PRO_FUNCAO` VARCHAR(100) NOT NULL,
  `FK_END_IDEND` INT NOT NULL,
  `FK_TEL_TELID` INT(11) NOT NULL,
  PRIMARY KEY (`PK_PRO_CPF`),
  INDEX `fk_Profissional_Endereco1_idx` (`FK_END_IDEND` ASC),
  INDEX `fk_Profissional_Telefone1_idx` (`FK_TEL_TELID` ASC),
  CONSTRAINT `fk_Profissional_Endereco1`
    FOREIGN KEY (`FK_END_IDEND`)
    REFERENCES `mydb`.`Endereco` (`PK_END_IDEND`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_Profissional_Telefone1`
    FOREIGN KEY (`FK_TEL_TELID`)
    REFERENCES `mydb`.`Telefone` (`PK_TEL_TELID`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
